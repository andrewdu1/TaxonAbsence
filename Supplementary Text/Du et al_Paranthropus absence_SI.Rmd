...
In Bayesian statistics, this commonly used distribution is referred to as the *beta-binomial distribution* and has the following probability mass function (PMF):
...
It is numerically equivalent to the *joint probability* of the data given the parameters, so to get the likelihood function, we multiply the probability of the data by itself $N$ times, where $N$ is sample size [@wang_principles_2010]: e.g., $L(\psi, \lambda | X) = \prod_{i}^{N} P(X_i | \psi, \lambda)$. 
For our study, the probability of the data is given by Equation \ref{P_X}, so our likelihood function is defined as:

\begin{align}
    L(\psi, \lambda | X) = P(X | \psi, \lambda) = \prod_{i}^{N} I(x_i = 0)(1 - \psi) + f_B(x_i; n_i, \lambda) \psi.
    \label{likelihood}
\end{align}

We then use the L-BFGS-B optimization algorithm [@byrd_limited_1995] to find the parameter values $(\psi, \lambda)$ that maximizes the likelihood function (Equation \ref{likelihood}); more accurately, we work with the log transformation of Equation \ref{likelihood} for analytical tractability.


## Bayes' rule

Once the $\psi$ and $\lambda$ parameters are estimated, we use Bayes' rule to compute our main probability of interest: the posterior probability that *Paranthropus* was truly absent at the $i$th site (i.e., $Z_i = 0$), given the estimated parameters and data.
...
\begin{aligned}
    P(Z_i = 0 | x_i = 0, n_i, \hat{\psi}, \hat{\lambda}) &= \frac{P(X_i = 0 | Z_i = 0, n_i, \hat{\psi}, \hat{\lambda}) P(Z_i = 0 | n_i, \hat{\psi}, \hat{\lambda})}{P(X_i = 0 | n_i, \hat{\psi}, \hat{\lambda})} \\
               &= \frac{I(x_i = 0) (1 - \hat{\psi})}{I(x_i = 0) (1 - \hat{\psi}) + f_B(x_i = 0; n_i, \hat{\lambda}) \hat{\psi}} \\
               &= \frac{1 \times (1 - \hat{\psi})}{1 \times (1 - \hat{\psi}) + \frac{\Gamma(1) \Gamma(n_i + 1 + \hat{\lambda})}{\Gamma(n_i + 2 + \hat{\lambda})} \frac{\Gamma(2 + \hat{\lambda})}{\Gamma(1) \Gamma(1 + \hat{\lambda})} \hat{\psi}} \\
...
We then plug in our estimated parameters $(\hat{\psi}, \hat{\lambda})$ and data for the $i$th site (i.e., the number of sampled mammalian specimens, $n_i$) into Equation \ref{post_prob} to get our posterior probability of interest.
...
To assess our model's goodness of fit and whether it is overly complex (e.g., too many parameters), we compared our model to others using the small-sample, bias-corrected Akaike Information Criterion (AIC~*c*~).
...
The number of simulated iterations where $\hat{\psi} = 1.0$ is itself caused by those iterations where there are many simulated zeros, i.e., number of zero *Paranthropus* specimens across sites.
