...
Therefore, we model $\pi_i$ using the standard reflected beta distribution [@wangAdaptiveCredibleIntervals2016], which has the probability density function $(1 - \lambda) (1 - \pi_i) ^ {-\lambda}$ if $\lambda\leq 0$. 
...
This is appropriate here because such a distribution has the majority of its probability density concentrated towards lower values of $\pi_i$, reflecting the general ecological pattern that a species is rare at most sites within its geographic range [@brownSpatialVariationAbundance1995; @murraySpeciesTailRankabundance1999].
...
```{r, echo = FALSE}
d <- read.csv("C:/Users/adu/OneDrive - Colostate/Desktop/Manuscripts/Du & Alemseged 2019_Paranthropus afar/GitHub/TaxonAbsence/Datasets/NISP data.csv", header = TRUE)
```
...
```{r, echo = FALSE}
# source functions
source("C:/Users/adu/OneDrive - Colostate/Desktop/Manuscripts/Du & Alemseged 2019_Paranthropus afar/GitHub/TaxonAbsence/Code/1_R functions.R")
...
Chosen values for $p$ are (`r psi.sim`), and those for $\lambda$ are (`r lambda.sim`).
...
All of this is operationalized with the `simulateData` function in our R script file. Note that we use R v.3.6.1 [@rcoreteamLanguageEnvironmentStatistical2021] for all our analyses.
...
save(sim.res, file = "C:/Users/adu/OneDrive - Colostate/Desktop/Manuscripts/Du & Alemseged 2019_Paranthropus afar/GitHub/TaxonAbsence/Supplementary Text/Model fits on simulations_2-16-22.RData")
```

```{r, echo = FALSE}
load("C:/Users/adu/OneDrive - Colostate/Desktop/Manuscripts/Du & Alemseged 2019_Paranthropus afar/GitHub/TaxonAbsence/Supplementary Text/Model fits on simulations_2-16-22.RData")
...
Overall, however, the relative bias for $\hat{p}$ is small (range: `r round(min(psi.bias.mean), 2)` to `r round(max(psi.bias.mean), 2)`).
...
When using the median parameter estimate, instead of the mean, across all `r n.iter` iterations for each pairwise combination of $p$ and $\lambda$, the relative bias in $\hat{p}$ is generally small across all parameter values (range: `r round(min(psi.bias.median), 2)` to `r round(max(psi.bias.median), 2)`) (Figure \ref{fig:sim}).
...
Either way, our main analysis gives $\hat{p}$ = `r round(psi_hat, 2)`, where $\hat{\lambda}$ is expected to have a negligible bias (Figure \ref{fig:sim}).
