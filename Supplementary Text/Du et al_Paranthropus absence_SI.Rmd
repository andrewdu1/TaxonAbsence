...
Bayes' theorem is a mathematical equation that allows one to compute the probability of an event conditioned on information which may impact that event. 
In this work, we are interested in computing the probability that *Paranthropus* was truly absent at a site ($Z_i$), conditioned on the fact that $X_i$ *Paranthropus* specimens have been found after sampling $n_i$ mammalian specimens. 
We call this probability the *posterior probability* and write it as $P(Z_i|X_i)$. 
However, it is much more mathematically tractable to compute the probability of sampling $X_i$ *Paranthropus* specimens conditioned on whether *Paranthropus* was present or not $P(X_i|Z_i)$; this probability is called the *likelihood*. 
Bayes' theorem allows us to express $P(Z_i|X_i)$ in terms of $P(X_i|Z_i)$:
...
In this equation, $P(Z_i)$ is called the *prior* and reflects our initial belief, before seeing any evidence, that *Paranthropus* was present at any site chosen at random. 
...
Bayes' theorem has given rise to an entire branch of statistical inference called *Bayesian inference*. 
For a full treatment, see Gelman et al. [-@gelman_bayes_2014].

### Applying Bayes' theorem to our research question

Once our model's $\psi$ and $\lambda$ parameters are estimated (Section \ref{estim}), we use Bayes' theorem (Eq. \ref{bayes}) to compute our main probability of interest: the posterior probability that *Paranthropus* was truly absent at the $i$th site (i.e., $Z_i = 0$), given the estimated parameters and data.
...
With Equation \ref{post_prob}, we plug in our estimated parameters $(\hat{\psi}, \hat{\lambda})$ and data for the $i$th site (i.e., the number of sampled mammalian specimens, $n_i$) to get our posterior probability of interest.
